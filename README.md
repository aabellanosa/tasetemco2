...
Loan Collateral Tracking v1 adds an internal management-only collateral type to loan applications. Loan Officer selects `PDC`, `ATM Cards`, or `Payroll`; the value is visible in the application and credit-review screens but is intentionally excluded from the printed loan application form. This adds the `loan_applications.collateral_type` Postgres column, so run `npm run pg:migrate` before deploying the app build.

Loan Document Form v1 adds a `Prepare Form` action to loan applications. Loan Officer can save the editable paper-form fields that are not already part of the application record, including form category, borrower address, spouse name, manual co-maker name, promissory note number, place signed, and internal routing notes. Category defaults follow the paper form: Emergency Loan maps to Emergency, Petty Cash Loan maps to Other, Small Business Loan maps to Entrepreneurial, and Salary/Educational/Appliance loans map to Providential. This phase stores the data needed for the future printable paper-form clone while leaving the existing loan workflow unchanged. It adds the `loan_document_forms` Postgres table, so run `npm run pg:migrate` before deploying this app build.

Loan Document Form v2 adds `Print Paper Form` from the Prepare Form modal. The printout pre-fills the loan application paper form with borrower/application details, form category, deductions, net proceeds, acknowledgement text, logo, and wet-signature lines. The amortization schedule remains printed separately from saved loan computations as the attachment.

Loan Application Balance Snapshot v1 records the member's manual previous-loan balance, outstanding system-loan balance, CBU/share capital, regular savings, and secured savings when a loan application is created. Credit Review and the replicated printable loan form show these application-date figures; secured savings remains visible when zero. `Previous Loan Balance` is the sum of the manual and system balances. Later member transactions and draft edits do not rewrite the snapshot. Existing database deployments must run `npm run pg:migrate` before deploying this application build.
...
Loan Portfolio Watch v1 seeds one demo posted loan with an overdue installment and one near-due installment so collection follow-up can be demonstrated after reset/seed. The dashboard shows the detailed member list only to management, loan, accounting, audit, and executive roles; Teller/Cashier and Membership Officer do not receive portfolio-wide overdue borrower details.
...
Membership applications capture gender, presented ID type and number, spouse name (when applicable), and `Required Initial Share Capital` as the expected membership requirement. ID type uses a controlled list of common Philippine identification documents. On approval, the identity and spouse fields carry into the active member record, where authorized profile editors can view and update them. Teller/Cashier records the actual opening payment for share capital, membership fee, and savings after System Administrator or General Manager approval. Existing database deployments must run `npm run pg:migrate` before deploying this application build.
...
The provisional financial statements now derive Consumers sales from posted Daily Remittance (WRS, G-mars, Catering, Load, GCash, and POS) and use finalized monthly inventory sheets for Canteen A, Canteen B, Bodega, and Consumer. See `docs/WORKFLOW.md` for the report rules and unresolved-data behavior.

The Teller / Cashier has a **Monthly Contributions** workspace for TFEA, CBU, and Secured Savings funded by either Cash Payment or Payroll Deduction. A receipt or payroll reference is required. A saved Draft has no financial effect; adding it to the Open teller batch makes the total part of cashier collection and expected cash. After cash count and review, Bookkeeper posting debits Cash on Hand, credits TFEA Payable, Share Capital, and Secured Savings Payable, feeds SUMMO, and increases each member's CBU/share-capital balance. These amounts are not member payables.

The Teller / Cashier also has a **Daily Remittance** workspace for cash turned over by cooperative operations and service units. Its operational Remittance Date may be earlier than its Cash Received Date. Drafts have no cash or accounting effect; adding a Draft to the Open teller batch includes its total in expected cashier cash. Reviewed-batch posting debits Cash on Hand and credits each source's snapshotted income account. Canteen A/B retain their cost centers, while WRS and Water Bottle A/B roll up under WRS with source detail preserved. POS, GCASH, and LOADER are active standalone sources mapped to Other Operating Income. Standalone and Admin-configured additional sources retain their own reporting definitions. Daily Remittance does not create member payables or feed SUMMO. Run `npm run pg:migrate` before deploying this workflow.
