...
Loan Release v1a lets Teller/Cashier release only loans marked `For Release`. Teller confirms the release date, unique voucher/reference number, and cash released, which must exactly equal computed net proceeds. The release is immutable, linked to the current Open teller batch, counted as cash-out, and changes the loan and application status to `Released`. Loan Officer, Admin, Manager, and Auditor have read-only release visibility. Bookkeeper journal posting remains Loan Release v1b.
...
Loan Collection v1-v2 supports real receipt amounts against the earliest unpaid installment. Teller/Cashier sees only posted loans and the next collectible installment, records a unique official receipt/reference, and may accept a partial, full, or advance payment as long as the amount does not exceed the remaining loan balance. The UI previews the allocation before confirmation: interest is applied first, then principal, with any excess over the current installment treated as advance principal payment. Recording adds the receipt to the Open teller batch and updates installment status as `Partial` or `Paid`. Bookkeeper sees the same principal-applied and interest-applied split in the unposted teller batch and batch details before posting. After cash count and review, `Post reviewed batch` debits Cash on Hand, credits Loans Receivable for principal applied, and credits Interest Income for interest applied. Skipped-installment, penalty, payoff, and amortization recomputation policies are intentionally deferred until confirmed by TASETEMCO.
...
Daily Disbursement is the cash-out mirror of Daily Remittance. Teller/Cashier records a backdateable Disbursement Date, the actual Cash Disbursed Date, a unique voucher/reference, and positive amounts in the fixed category grid. Canteen A, Canteen B, and WRS retain cost-center attribution; Building and renovations, Travel, and Other expenses belong to cooperative operations. Admin may configure additional categories and revise reporting groups, cost centers, `5xxx` expense accounts, display order, and status. Adding a Draft to the Open teller batch includes its total as cash-out. Reviewed-batch posting debits the snapshotted expense accounts and credits `1010 - Cash on Hand`. Teller and Admin cash-position views use the same active batch ID, so their transaction counts and rows reconcile. Run `npm run pg:migrate`; the migration seeds initial categories with `ON CONFLICT DO NOTHING`, so no separate seed or reset is required.
