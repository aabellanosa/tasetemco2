...
Membership applications capture gender, a searchable Philippine ID type, ID number, one beneficiary's name, whole-number age, relationship to the member, and `Required Initial Share Capital` as the expected membership requirement. Approval carries the beneficiary fields into the member profile, where authorized profile editors may maintain them. Either the System Administrator or General Manager may approve a submitted membership application. Teller/Cashier records the actual opening payment for share capital, membership fee, and savings after approval. Beneficiary details are member master data and are not printed in the current loan packet because its source form has no beneficiary section.
