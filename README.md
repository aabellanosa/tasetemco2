...
The System Administrator and General Manager manage staff accounts under Users. New accounts start as `Pending Activation` with a unique temporary password that is displayed only once. The user cannot enter operational screens until replacing it with a compliant password. Both roles can maintain ordinary staff roles/default screens, set accounts to Active, Locked, or Disabled, and issue a fresh temporary password. Only the System Administrator can assign the System Administrator role or manage an account holding that role; demo/database maintenance also remains Admin-only. Locking, disabling, and password reset invalidate the user's existing sessions. Recent creation, update, login, password-change, and reset events are visible in the account security trail.
...
CBU Withdrawal v1 lets Teller/Cashier encode a member-requested withdrawal of share capital without a separate Manager approval. The server permits only the amount that leaves `PHP 5,000 membership minimum + positive manual previous-loan exposure + remaining Posted system-loan exposure + pending CBU withdrawals`. A violation is blocked in the API and shown in a dedicated Teller warning that directs the transaction to the System Administrator. If Teller cash is sufficient, the withdrawal enters the Open batch immediately; otherwise it remains `For Funding`, contributes to Bookkeeper funding demand, and Teller releases it after acknowledged funding is available. Posting, not request entry, reduces the member CBU balance and creates debit `3010 - Share Capital`, credit `1010 - Cash on Hand`. Request, release, and posting actors and timestamps remain auditable. Run `npm run pg:migrate` before deploying this build because it adds the `cbu_withdrawals` table.

Loan Collection v1-v3 supports real receipt amounts against the earliest unpaid installment. Teller/Cashier sees only posted loans and the next collectible installment, records a unique official receipt/reference, and may accept a partial, full, or advance payment as long as the amount does not exceed the remaining loan balance plus assessed penalties. The UI previews every affected installment before confirmation. An unpaid scheduled amount receives a 2% penalty after a seven-calendar-day grace period; a partial payment made before assessment reduces the penalty base to the remaining unpaid portion. Penalties remain separate and do not compound. Collection applies assessed penalties separately before allocating remaining payment to the oldest installment's interest and principal, then future installments. One receipt retains its penalty and installment allocations while entering the Open teller batch as one cash-in transaction. After review, posting debits Cash on Hand and credits Loans Receivable, Interest Income, and the assessment's snapshotted `4040 - Penalty Income` amount separately.
...
Cost Center Dues Collection v1 complements Cost Center Charges. Teller/Cashier selects an Active member and sees outstanding dues separately for Canteen A, Canteen B, WRS, G-mar, and other configured centers. Teller explicitly allocates the receipt among centers; the server applies each center allocation to its oldest outstanding charge movements first. Partial payment is allowed, overpayment is rejected, and an unposted Teller payment reserves the balance against duplicate collection. The receipt enters the Open Teller batch as cash-in. Reviewed-batch posting debits `1010 - Cash on Hand` and credits the snapshotted income account associated with the cost center's active remittance source; a center without a mapped source uses `4080 - Other Operating Income`. Member profile and payment history retain the original charges, payment allocations, remaining dues, receipt, Teller, batch, Bookkeeper, and journal entry. Cost Center Payable Movements preserves `Charge` or `Reversal` as the immutable movement type and separately shows Original, Paid, Balance, and derived `Unpaid`, `Partially Paid`, `Paid`, or `Reversed` status. Run `npm run pg:migrate` before deploying because this workflow adds payment and allocation tables.
...
Membership applications require one and allow up to three beneficiaries, each with a name, whole-number age from 0 to 150, and relationship to the member. They also capture gender, a searchable Philippine ID type, ID number, spouse name (when applicable), and `Required Initial Share Capital` as the expected membership requirement. The spouse input appears immediately before beneficiaries. Approval carries the spouse and all beneficiary records into the member profile, where authorized profile editors may maintain them. Beneficiaries are stored as separate application and member child records; an existing legacy beneficiary is retained as the first record during migration. Either the System Administrator or General Manager may approve a submitted membership application. Teller/Cashier records the actual opening payment for share capital, membership fee, and savings after approval. Beneficiary details are member master data and are not printed in the current loan packet because its source form has no beneficiary section.
...
Cash-in OR/reference numbers are unique across initial member payments, share capital contributions, savings deposits, and loan collections. Withdrawal voucher/reference numbers are unique across savings, secured-savings, and CBU withdrawals.
...
The current application also exposes **Reports → Provisional FSC & FSO**. It combines system-derived, controlled manual, carried-forward, calculated, and unresolved values; shows the real FSC balance difference; tracks client questions; blocks premature finalization; and exports an explicitly provisional three-sheet Excel workbook. Consumers sales come from posted Daily Remittance for WRS, G-mars, Catering, Load, GCash, and POS. Finalized monthly inventory drives beginning inventory, ending inventory, and the client-approved Merchandise Inventory rule. See `docs/WORKFLOW.md` section 5.1 and the Reports section of the static workflow site for the migration and acceptance-test steps.

Monthly Inventory provides persistent PostgreSQL-backed sheets for Canteen A, Canteen B, Bodega, and Consumer. Canteen A and B each use the same 426 supplied item rows, grouped into seven classifications; source duplicates remain separate coded items. Bodega uses its confirmed 50-item June 2026 catalog, grouped into seven Bodega-specific school and office supply classifications, replacing the ten prototype items. No quantities or prices are prefilled. Authorized users can add a catalog item for a location and classification. Each monthly sheet captures beginning inventory, purchases, transfers in/out, ending inventory, and unit price; it calculates TGAS, units sold/used, ending value, classification subtotals, and a grand total. Download Excel exports the selected location/month and its current grid values, including unsaved edits, as a grouped workbook with subtotals, a grand total, frozen/repeating headings, landscape orientation, and fit-to-one-page-wide printing. Drafts may be saved and finalized, while authorized corrections require reopening with a reason and retain audit events. System Administrator and General Manager have full inventory authority; Accountant/Bookkeeper can encode and finalize; Teller/Cashier can encode; Auditor has detailed read-only access; Board receives summary-only access. General Manager's expanded authority applies only inside Inventory. Finalized sheets supply the provisional financial-statement inventory rules but do not create GL entries. Run `npm run pg:migrate` before deployment.
