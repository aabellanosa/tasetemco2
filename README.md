...
Loan Document Form v1 adds a `Prepare Form` action to loan applications. Loan Officer can save the editable paper-form fields that are not already part of the application record, including borrower address, spouse name, manual co-maker name, promissory note number, place signed, and internal routing notes. This phase stores the data needed for the future printable paper-form clone while leaving the existing loan workflow unchanged. It adds the `loan_document_forms` Postgres table, so run `npm run pg:migrate` before deploying this app build.
