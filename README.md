...
Member Profile v1 expands member master data with contact number, address, birthdate, gender, civil status, occupation/source of income, membership date, cluster/group, Philippine ID type, ID number, one to three beneficiaries (each with name, age, and relationship), and status. ID type uses the same searchable selection available in membership applications. `admin` and `membership` can update profile fields; other member-view roles can inspect them read-only. Share capital and savings balances stay read-only because they are derived from transactions.
