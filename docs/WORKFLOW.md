...
Loan Document Form v1 adds a `Prepare Form` action to loan applications. Loan Officer can save editable paper-form fields attached to the application, including borrower address, spouse name, manual co-maker name, promissory note number, place signed, and internal bookkeeper/approval notes. This is the first digital-form phase: it captures the extra fields needed for the later printable paper-form clone while keeping the existing Draft, Submit, Review, Compute, Release, and posting workflow unchanged.
