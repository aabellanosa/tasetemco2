...
Loan Document Form v1 adds a `Prepare Form` action to loan applications. Loan Officer can save editable paper-form fields attached to the application, including form category, borrower address, spouse name, manual co-maker name, promissory note number, place signed, and internal bookkeeper/approval notes. The category defaults from the selected product: Emergency Loan maps to Emergency, Petty Cash Loan maps to Other, Small Business Loan maps to Entrepreneurial, and Salary/Educational/Appliance loans map to Providential. This is the first digital-form phase: it captures the extra fields needed for the later printable paper-form clone while keeping the existing Draft, Submit, Review, Compute, Release, and posting workflow unchanged.

Loan Document Form v2 adds `Print Paper Form` from the same Prepare Form modal. The printout uses the saved/form-entered fields plus the application snapshot to prefill borrower details, loan category, amount, term, purpose, bookkeeper notes, approval/routing notes, disbursement deductions, net proceeds, acknowledgement text, logo, and wet-signature lines. It is a printable replacement for preparing the paper form, while the amortization schedule remains a separate printable attachment from the computation screen.
...
The `admin` user has a Demo Maintenance panel under System Setup. This panel shows Postgres table counts, downloads a JSON backup, and can reset hosted demo data to the seed rows. Reset requires the typed confirmation `RESET TASETEMCO` and downloads a pre-reset backup automatically.
...
Loan Document Form v1 adds the `loan_document_forms` Postgres table. Run `npm run pg:migrate` before starting or deploying this build. No seed is required. Existing loan applications remain valid; their document-form data starts blank except for defaults that can be derived from the member/application record. If the app code is deployed before this migration, `Prepare Form` returns a clear migration message instead of a generic server error.
