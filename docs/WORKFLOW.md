...
Membership applications require one and allow up to three beneficiaries, each with a name, whole-number age from 0 to 150, and relationship to the member. They also capture gender, a searchable Philippine ID type, ID number, and `Required Initial Share Capital` as the expected membership requirement. Either the System Administrator or General Manager may approve a submitted membership application. Approval copies all beneficiary records into the member profile. Beneficiaries use separate application and member child records, with an existing legacy beneficiary backfilled as the first record. Teller/Cashier records the actual opening payment for share capital, membership fee, and savings after approval. The System Administrator and Membership Officer can view and edit gender, ID details, and beneficiary details in the member profile. Beneficiary data is not included in the current loan packet because the replicated form contains no beneficiary section.
...
Member Profile v1 adds editable master-data fields for contact number, address, birthdate, gender, civil status, occupation/source of income, membership date, cluster/group, Philippine ID type, ID number, one to three beneficiaries (each with name, age, and relationship), and status. ID type uses the same searchable selection available in membership applications. The System Administrator and Membership Officer can update these fields. Manager, Auditor, and other member-view roles can review the profile read-only. Financial balances remain transaction-derived and cannot be edited from the profile panel.
