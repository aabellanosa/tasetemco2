...
Membership applications require one and allow up to three beneficiaries, each with a name, whole-number age from 0 to 150, and relationship to the member. They also capture gender, a searchable Philippine ID type, ID number, spouse name (when applicable), and `Required Initial Share Capital` as the expected membership requirement. The spouse input appears immediately before beneficiaries. Either the System Administrator or General Manager may approve a submitted membership application. Approval copies the spouse and all beneficiary records into the member profile. Beneficiaries use separate application and member child records, with an existing legacy beneficiary backfilled as the first record. Teller/Cashier records the actual opening payment for share capital, membership fee, and savings after approval. The System Administrator and Membership Officer can view and edit gender, ID, spouse, and beneficiary details in the member profile. Beneficiary data is not included in the current loan packet because the replicated form contains no beneficiary section.
...
Cost Center Payments is the settlement counterpart of Cost Center Charges. Finalized charge movements establish what the member owes. Teller selects the member, reviews outstanding balances per cost center, and explicitly allocates cash among one or more centers. Each allocation settles the oldest outstanding charge within that center first; partial payments are allowed and payments above the remaining balance are rejected. Teller Batch status reserves the allocated dues immediately. After cash count and Bookkeeper review, posting debits `1010 - Cash on Hand` and credits the cost center's snapshotted collection income account. The member statement preserves both charge and payment histories and shows the remaining net dues. Its payable-movement table keeps the source Type unchanged and adds Original, Paid, Balance, and Payment Status columns so full and partial settlement remain visible without weakening the audit trail.

Teller/Cashier can record a CBU withdrawal only when the remaining posted CBU covers the PHP 5,000 membership minimum plus manual and system loan exposure. Pending CBU requests reserve available capital. Requests without sufficient Teller cash wait in `For Funding`; no separate Manager approval is introduced.
...
The Inventory workspace provides persistent monthly sheets for Canteen A, Canteen B, Bodega, and Consumer. PostgreSQL stores the item catalog, classifications, location assignments, sheets, rows, statuses, actors, timestamps, correction reasons, and audit events. Canteen A and Canteen B each start with the same 426-item catalog supplied in the client's June 2026 Canteen A workbook. The catalog retains the source item wording and separately coded duplicate rows, organized into seven classifications. Bodega starts with the 50-item catalog from the client's June 2026 Bodega workbook, organized into seven Bodega-specific school and office supply classifications. Its four description columns are combined into distinguishable item names, and the ten prototype starter items are retired. Quantities and prices are deliberately not seeded for any location. Authorized catalog managers can add an item for a selected location and classification.
...
The screen groups items by classification, shows classification subtotals and a grand total, and highlights a row when transfers exceed available units or ending inventory exceeds TGAS. Separate Transfer In and Transfer Out fields replace a direction-specific “Withdrawal from B to A” column so the same grid can eventually support movements among all three locations. Download Excel exports the current grid for the selected location and month, including unsaved entries. The workbook contains the cooperative heading, status, preparer, all grouped items, classification subtotals, and the grand total. It is configured for landscape, fit-to-one-page-wide printing with frozen and repeating headings, allowing staff to review or adjust page breaks in Excel before printing or saving as PDF.
...
The persistent module does not post accounting entries. Its finalized values now supply the provisional financial statements: the prior month's ending value becomes FSO Beginning Inventory, the current month's ending value becomes FSO Ending Inventory, and the current month's combined beginning value supplies FSC Merchandise Inventory under the client's approved template rule. All four locations must be finalized; otherwise the affected statement line stays visibly unresolved. Deployments require `npm run pg:migrate`.
...
Daily Disbursement v1 mirrors Daily Remittance as a fixed-grid cash-out Draft. Admin configures category name, reporting group, optional cost center, `5xxx` expense account, display order, and status. The standard categories are Canteen A, Canteen B, WRS, Building and renovations, Travel, Water, Light, Wifi, SSS, Pag-ibig, Philhealth, and Other expenses. `Other expenses` is deliberately displayed last. Canteen A/B and WRS retain cost-center attribution; the other standard categories belong to cooperative operations. Teller records a backdateable Disbursement Date, actual Cash Disbursed Date, globally unique voucher/reference, positive category amounts, and optional remarks. Adding the Draft to the Open teller batch includes its total in cash-out. After cash count and Bookkeeper review, posting debits the snapshotted expense accounts and credits `1010 - Cash on Hand`. The Teller cash position and Admin Outstanding Teller Batch use the same active batch ID and therefore must show matching transaction rows and counts.
...
9. Canteen A/B retain cost centers C1/C2 and WRS retains cost center WRS. Building and renovations, Travel, Water, Light, Wifi, SSS, Pag-ibig, Philhealth, and Other expenses remain attributed to cooperative operations; Other expenses is displayed last.
...
Consumers sales are calculated from posted Daily Remittance entries for WRS (including Water Bottle A/B), G-mars, Catering, Load, GCash, and POS. Draft and Teller Batch remittances are excluded. GCash and Load sales are not repeated as an FSC asset balance; a separate supported wallet balance would be required for that treatment.

FSO Beginning Inventory uses the previous month's finalized ending inventory, and FSO Ending Inventory uses the current month's finalized ending inventory. Both totals require Canteen A, Canteen B, Bodega, and Consumer. FSC Merchandise Inventory follows the client's current template instruction and totals the current month's beginning-inventory value for those four locations. Missing or unfinalized locations leave the corresponding line unresolved rather than producing a partial total.
...
**Deployment:** Run `npm run pg:migrate` against each local or hosted PostgreSQL target. This adds the financial-statement tables when absent and inserts the standard GMAR remittance-source definition without resetting operational data. Do not run `pg:seed` or `pg:reset-demo` for this change. In memory mode the workflow is testable, but saved periods disappear when the backend restarts.
...
Daily Disbursement adds `disbursement_categories`, `daily_disbursement_batches`, and `daily_disbursement_entries`. The migration seeds Canteen A, Canteen B, WRS, Building and renovations, Travel, Water, Light, Wifi, SSS, Pag-ibig, Philhealth, and Other expenses using conflict-safe inserts, then moves Other expenses to the last standard display position. Run `npm run pg:migrate` after deploying the application build. No separate `pg:seed` or database reset is required.
